...
# Visualization
def draw_graph_dfs(graph, path, rotate_angle=0):
    G = nx.Graph()
    ...
    plt.title("DFS Search Path Visualization", fontsize=12, fontweight='bold', color='darkblue')
    
    # Display path under the graph
    if path:
        plt.figtext(0.5, 0.01, f"DFS Path: {path}", wrap=True, horizontalalignment='center', fontsize=12, color='darkblue')
    
    plt.show()

# Draw the graph with DFS path
draw_graph_dfs(graph, path, rotate_angle=90)
